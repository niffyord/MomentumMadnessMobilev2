...
// Responsive design tokens
      ...
      >
        ...
        <View style={styles.winnerContent}>
          ...
          <Text style={styles.winnerPerformance}>
            Final Performance: {winnerAsset?.performance >= 0 ? '+' : ''}{(typeof winnerAsset?.performance === 'number' && !isNaN(winnerAsset.performance)) ? winnerAsset.performance.toFixed(2) : '0.00'}%
          </Text>
          ...
          <View style={styles.winnerStatsGrid}>
            ...
            <View style={styles.winnerStatItem}>
              <Text style={[
                ...
                {(typeof raceResults?.performanceSpread === 'number' && !isNaN(raceResults.performanceSpread)) ? raceResults.performanceSpread.toFixed(1) : '0.0'}%
              </Text>
  ...
  function _EnhancedSettledPhase({ 
    ...
    // Import the new robust claim payout hook
    const claimPayoutMutation = useClaimPayout()
    const { userBets, fetchSettledPhaseData } = useRaceStore();
  
    // Fetch final race data once when race or account changes
    React.useEffect(() => {
    ...
    // After claiming payout, refresh final data
    React.useEffect(() => {
    ...
    // Enhanced animation refs with cleanup
    ...
    // New micro-interaction animation refs
    ...
    // Haptic feedback state to prevent overwhelming
    const lastHapticTime = useRef(0)
    const hasTriggeredEntryHaptic = useRef(false)
    // Local claim update state for immediate UI feedback (like v1)
    const [localClaimUpdate, setLocalClaimUpdate] = useState(false)
  
    // Get the claiming status from the hook
    const isClaimingPayout = claimPayoutMutation.isPending
  
    // Enhanced haptic feedback helper with debouncing
    const triggerHaptic = useCallback(async (type: 'light' | 'medium' | 'heavy' | 'success' | 'error' | 'selection') => {
      const now = Date.now()
      if (now - lastHapticTime.current < 100) return // Debounce haptics
    ...
    // Performance: Cleanup animations on unmount
    useEffect(() => {
      return () => {
    ...
    // ACCURATE asset performance calculations matching smart contract logic
    const assetPerformances = useMemo(() => {
      ...
      return race.assets.map((asset: any, index: number) => {
        ...
        // Safe performance calculation with fallback for new races
        let performance = 0
    ...
    // Enhanced winner determination (contract-accurate)
    const raceResults = useMemo(() => {
      ...
      const winnerAsset = winnerAssets[0] // Primary winner
      
      // Calculate race statistics
      const performanceSpread = Math.max(...assetPerformances.map((a: any) => a.performance)) - 
                               Math.min(...assetPerformances.map((a: any) => a.performance))
      const avgPerformance = assetPerformances.reduce((sum: number, a: any) => sum + a.performance, 0) / assetPerformances.length
      
      // Determine race intensity based on final results
      const raceIntensity = performanceSpread > 5 ? 'extreme' : 
                           performanceSpread > 2 ? 'high' : 
                           performanceSpread > 1 ? 'medium' : 'low'
    ...
    // Computed claimed status (like v1 pattern)
    ...
    // ACCURATE user position calculation matching smart contract
    const userPosition = useMemo(() => {
      ...
      const originalAmount = derivedUserBet.amount / 1_000_000 // Convert to USDC
      const isActualWinner = raceResults.winnerAssets.some((w: any) => w.index === derivedUserBet.assetIdx)
      
      // Calculate actual payout based on contract logic
      let actualPayout = 0
      ...
      if (isActualWinner) {
        ...
        const feeRate = 0.05 // 5% protocol fee
        ...
        if (winningPool > 0 && totalPool > 0) {
          ...
          actualPayout = claimableAmount - originalAmount // Profit
        }
      }
    ...
    // Enhanced entry haptics and animations based on user result
    useEffect(() => {
      if (userPosition && !hasTriggeredEntryHaptic.current) {
        hasTriggeredEntryHaptic.current = true
        // Different haptic patterns based on outcome
        if (isWinner) {
          // Victory haptic sequence
          ...
          // Participation haptic (gentler)
          setTimeout(() => triggerHaptic('light'), 200)
        }
      }
    }, [userPosition, isWinner, triggerHaptic])
  
    // User result animation - enhanced with spring physics
    useEffect(() => {
      if (userPosition) {
        ...
        if (isWinner && !reduceMotion) {
          ...
          // Glow animation for winners only
    ...
    // Enhanced leaderboard reveal with stagger animation
    useEffect(() => {
      if (showFullResults) {
        ...
        const leaderboardAnimation = Animated.spring(leaderboardAnim, {
          ...
          useNativeDriver: true,
               ...
               // Stagger animation for individual result rows
    ...
    // Reset local claim state when race changes (like v1 pattern)
    useEffect(() => {
    ...
    // Enhanced claim payout function with haptics
    const handleClaimPayout = useCallback(async () => {
      ...
      try {
        ...
        // Set local update immediately for UI feedback (like v1)
    ...
    // Enhanced sharing function with haptics
    const handleShare = useCallback(async () => {
      ...
      try {
        ...
        let shareTitle = ''
            ...
            // Winner sharing - similar to v1 but enhanced
            const returnPercentage = userPosition.originalAmount > 0 ? 
              (userPosition.actualPayout / userPosition.originalAmount) * 100 : 0
            
            shareTitle = '🏆 I WON on Momentum Madness!'
            shareMessage = `I just won $${userPosition.claimableAmount.toFixed(2)} with a +${returnPercentage.toFixed(0)}% return! 🚀\n\n` +
            ...
            // Participant sharing - new feature beyond v1
            shareTitle = '🏁 I raced on Momentum Madness!'
            shareMessage = `I just competed in a ${raceResults?.raceIntensity} intensity race! 🏎️\n\n` +
          ...
          // Spectator sharing - completely new feature
          shareTitle = '👀 I watched an epic race!'
          shareMessage = `Just watched an intense ${raceResults?.raceIntensity} race on Momentum Madness! 🏁\n\n` +
            `🏆 Winner: ${winnerAsset?.symbol} (+${winnerAsset?.performance.toFixed(2)}%)\n` +
    ...
    // Enhanced button press handlers with micro-interactions
    ...
    return (
      <View style={styles.settledContainer}>
        <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
          {/* Enhanced Race Winner Announcement */}
          <WinnerAnnouncement 
          ...
          {/* Enhanced User Results or Spectator Message */}
          ...
          >
            ...
            >
              ...
              {/* Enhanced Bet Summary with Accurate Calculations */}
              <View style={styles.betSummarySection}>
                ...
                <View style={styles.betSummaryRow}>
                  ...
                  <Text style={styles.betSummaryText}>
                    ${userPosition?.originalAmount.toFixed(2)}
                  </Text>
                </View>
                ...
                <View style={styles.betSummaryRow}>
                  ...
                  <Text style={[
                    ...
                    {userPosition?.finalPerformance >= 0 ? '+' : ''}{(typeof userPosition?.finalPerformance === 'number' && !isNaN(userPosition.finalPerformance)) ? userPosition.finalPerformance.toFixed(2) : '0.00'}%
                  </Text>
                </View>
                ...
                <View style={styles.betSummaryRow}>
                  ...
                  <Text style={[
                    ...
                    {(userPosition?.performanceVsWinner || 0) >= 0 ? '+' : ''}{(userPosition?.performanceVsWinner || 0).toFixed(2)}%
                  </Text>
                </View>
                ...
                <View style={styles.positionSummary}>
                  <Text style={styles.positionSummaryText}>
                    {userPosition?.isWinner ? 'Winner! ' : 'Participated in race with '}{userPosition?.totalParticipants} total racers
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.actionButtonsSection}>
                {isWinner && claimed ? (
                  /* Enhanced Claimed Status Display - Premium Design */
                  (() => {
                  ...
                  /* Enhanced Claim Button with Animations */
                  (() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <LinearGradient
                            colors={isClaimingPayout ? ['rgba(0, 255, 136, 0.5)', 'rgba(255, 215, 0, 0.5)'] : ['#00FF88', '#FFD700']}
                          ...
                          >
                            ...
                            <Text style={styles.claimButtonText}>
                              {isClaimingPayout ? 'Claiming...' : `Claim $${userPosition?.claimableAmount.toFixed(2)}`}
                            </Text>
                          </LinearGradient>
                        </TouchableOpacity>
                      </Animated.View>
                ...
                {/* Enhanced Share Button for Winners/Participants */}
                <Animated.View
                  style={{
                ...
                <View style={styles.toggleButtonsRow}>
                  ...
                  >
                    <Text style={styles.detailsButtonText}>
                      {showFullResults ? 'Hide' : 'Show'} Results
                    </Text>
                    <MaterialCommunityIcons 
                  ...
                  >
                    <Text style={styles.detailsButtonText}>
                      {showDetailedAnalytics ? 'Hide' : 'Show'} Analytics
                    </Text>
                    <MaterialCommunityIcons 
            ...
            /* Enhanced Spectator Results for Users Without Bets */
            <View style={styles.spectatorCard}>
              ...
              >
                <View style={styles.spectatorContent}>
                  ...
                  {/* Enhanced Spectator Share Button */}
                  <Animated.View
                    style={{
                      transform: [{ scale: shareButtonPulseAnim }],
                    }}
          ...
          {/* Enhanced Full Results Breakdown with Stagger Animation */}
          {showFullResults && (
            <Animated.View 
              style={[
                  ...
                  return (
                    ...
                    >
                      <View style={[
                        styles.resultRank,
                        {
                          backgroundColor: rank === 1 ? '#FFD700' : 
                                         rank === 2 ? '#C0C0C0' : 
                                         rank === 3 ? '#CD7F32' : 'rgba(255,255,255,0.2)',
                        }
                      ]}>
                        <Text style={[
                      ...
                      <View style={styles.resultStats}>
                        ...
                        <Text style={styles.resultPoolShare}>
                          {asset.poolShare.toFixed(1)}% pool
                        </Text>
                      </View>
                    </Animated.View>
                  )
              ...
              >
                ...
                <View style={styles.analyticsGrid}>
                  ...
                  <View style={styles.analyticsItem}>
                    ...
                    <Text style={styles.analyticsValue}>
                      {raceResults?.avgPerformance.toFixed(2)}%
                    </Text>
        ...
        {/* Next Race Preview with Real Cooldown Data */}
  ...
  const styles = StyleSheet.create({
    ...
    // Winner Section - Responsive margins
    winnerSection: {
    ...
    // Enhanced Claimed Status Styles - Premium Design
    claimedContainer: {
      borderRadius: 16,
      overflow: 'hidden',
      borderWidth: 2,
